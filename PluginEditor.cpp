...
namespace
{
    ...
    const juce::Colour colGreen   (0xff3DED7A); // verde claro y luminoso (lejos de 0 dB)
    const juce::Colour colAmber   (0xffFFC400); // amarillo intenso (cerca de 0 dB)
    const juce::Colour colRed     (0xffFF453A);
    ...
    : juce::AudioProcessorEditor (&p), proc (p)
{
    ...
    // --- Presets y A/B ---
    presetBox.setColour (juce::ComboBox::backgroundColourId, colPanel);
    presetBox.setColour (juce::ComboBox::textColourId, colText);
    presetBox.setColour (juce::ComboBox::outlineColourId, colLine);
    ...
    auto setupTextButton = [this] (juce::TextButton& b, const juce::String& text)
    {
        b.setButtonText (text);
        b.setColour (juce::TextButton::buttonColourId, colPanel);
        b.setColour (juce::TextButton::buttonOnColourId, colPink);
        b.setColour (juce::TextButton::textColourOffId, colText);
        b.setColour (juce::TextButton::textColourOnId, juce::Colours::white);
        addAndMakeVisible (b);
    };

    setupTextButton (saveButton, "Guardar");
    saveButton.onClick = [this] { savePresetToFile(); };

    setupTextButton (loadButton, "Cargar");
    loadButton.onClick = [this] { loadPresetFromFile(); };

    setupTextButton (aButton, "A");
    aButton.setClickingTogglesState (false);
    aButton.onClick = [this] { setActiveSide (false); };

    setupTextButton (bButton, "B");
    bButton.setClickingTogglesState (false);
    bButton.onClick = [this] { setActiveSide (true); };

    // El slot A arranca con el estado inicial (por defecto o cargado por el host)
...
void PrismaEQAudioProcessorEditor::mouseDoubleClick (const juce::MouseEvent& e)
{
    ...
    if (idx >= 0)
    {
        // Doble clic sobre un punto existente: su ganancia vuelve a 0
    ...
    // Doble clic en un hueco vacío: activa la primera banda apagada ahí mismo (agregar punto)
    int freeBand = -1;

    for (int b = 0; b < prisma::numBands; ++b)
        if (! proc.getBand (b).on)
        {
            freeBand = b;
            break;
        }
    ...
    if (freeBand < 0)
        return; // las 6 bandas ya estan en uso

    const auto freqId = bandParamId (freeBand, "freq");
    const auto gainId = bandParamId (freeBand, "gain");
    const auto onId = bandParamId (freeBand, "on");

    beginGesture (freqId);
    setParam (freqId, juce::jlimit (20.0f, 20000.0f, xToFreq (e.position.x)));
    endGesture (freqId);

    beginGesture (gainId);
    setParam (gainId, yToGain (e.position.y));
    endGesture (gainId);
...
void PrismaEQAudioProcessorEditor::buildPresetMenu()
{
    presetBox.clear (juce::dontSendNotification);

    int count = 0;
    const auto* presets = prisma::getFactoryPresets (count);
    ...
    for (int i = 0; i < count; ++i)
    {
        ...
        if (juce::String (p.category) != lastCategory)
        {
            lastCategory = p.category;
            presetBox.addSectionHeading (lastCategory);
        }

        presetBox.addItem (p.name, i + 1); // los ID de ComboBox no pueden ser 0
    }
}

void PrismaEQAudioProcessorEditor::applyPreset (int presetIndex)
{
    int count = 0;
    const auto* presets = prisma::getFactoryPresets (count);
    ...
    for (int b = 0; b < prisma::numBands; ++b)
    {
        const auto& pb = preset.bands[b];

        auto apply = [this] (const juce::String& id, float v)
        {
            beginGesture (id);
            setParam (id, v);
            endGesture (id);
        ...
        apply (bandParamId (b, "type"), (float) pb.type);
        apply (bandParamId (b, "freq"), pb.freq);
        apply (bandParamId (b, "gain"), pb.gain);
        apply (bandParamId (b, "q"), pb.q);
        apply (bandParamId (b, "on"), pb.on ? 1.0f : 0.0f);
    }

    selectBand (selectedBand);
}

void PrismaEQAudioProcessorEditor::savePresetToFile()
{
    fileChooser = std::make_unique<juce::FileChooser> (
        "Guardar preset de Prisma EQ",
    ...
    fileChooser->launchAsync (flags, [this] (const juce::FileChooser& fc)
    {
        ...
        if (! file.hasFileExtension (".prismaeq"))
            file = file.withFileExtension (".prismaeq");
...
void PrismaEQAudioProcessorEditor::loadPresetFromFile()
{
    fileChooser = std::make_unique<juce::FileChooser> (
        "Cargar preset de Prisma EQ",
        juce::File::getSpecialLocation (juce::File::userDocumentsDirectory),
        "*.prismaeq");
...
void PrismaEQAudioProcessorEditor::setActiveSide (bool sideB)
{
    if (sideB == onSideB)
        return;

    // Guarda el estado actual en el lado que se abandona
    auto& outgoing = onSideB ? stateB : stateA;
    proc.getStateInformation (outgoing);

    onSideB = sideB;
    auto& incoming = onSideB ? stateB : stateA;

    if (incoming.getSize() > 0)
        proc.setStateInformation (incoming.getData(), (int) incoming.getSize());
    // Si el lado destino aun no tiene estado guardado (primer cambio a B),
    // se queda con el estado actual, como copia de partida.

    aButton.setToggleState (! onSideB, juce::dontSendNotification);
    bButton.setToggleState (onSideB, juce::dontSendNotification);
    presetBox.setSelectedId (0, juce::dontSendNotification);
    selectBand (selectedBand);
}
...
void PrismaEQAudioProcessorEditor::resized()
{
    ...
    const int dispH = juce::roundToInt ((float) b.getHeight() * 0.54f);
    displayArea = b.removeFromTop (dispH).toFloat();
    plotArea = displayArea.withTrimmedTop (40.0f).withTrimmedBottom (22.0f);

    auto presetRow = b.removeFromTop (30).reduced (12, 3);
    presetBox.setBounds (presetRow.removeFromLeft (presetRow.getWidth() * 40 / 100));
    presetRow.removeFromLeft (6);
    bButton.setBounds (presetRow.removeFromRight (30));
    presetRow.removeFromRight (4);
    aButton.setBounds (presetRow.removeFromRight (30));
    presetRow.removeFromRight (8);
    loadButton.setBounds (presetRow.removeFromRight (70));
    presetRow.removeFromRight (4);
    saveButton.setBounds (presetRow.removeFromRight (70));
